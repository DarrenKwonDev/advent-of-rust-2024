...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        if parts.len() != 3 {
            return Err("Invalid CSV format: expected 3 fields");
        }
        let name = parts[0].to_string();
        let good_deeds = parts[1].parse::<u32>()
            .map_err(|_| "Invalid good deeds number")?;
        let bad_deeds = parts[2].parse::<u32>()
            .map_err(|_| "Invalid bad deeds number")?;
